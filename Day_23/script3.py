...
def clean_ways(ways):
    for way in ways:
        target_positions = way['positions'][-50:]
        for other_way in ways:
            if way == other_way:
                continue
            positions = other_way['positions'][:-99]
            if target_positions[0] not in positions:
                continue
            if positions[positions.index(target_positions[0]):positions.index(target_positions[0])+len(target_positions)] == target_positions:
